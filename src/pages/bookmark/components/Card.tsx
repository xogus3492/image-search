import { CardDTO } from '@/pages/index/types/card'
import styles from './Card.module.scss'

interface Props {
    prop: CardDTO
}

function Card({ prop }: Props) {
  return (
    <div className={styles.card}>
        <div className={styles.card__imageBox}>
            <img src={prop.urls.small} alt='' className={styles.card__imageBox__image} />
        </div>
        <div className={styles.card__infoBox}>
            <div className={styles.card__infoBox__row}>
                <div className={styles.label}>작성자</div>
                <div className={styles.value}>{prop.user.name}</div>
            </div>
            <div className={styles.card__infoBox__row}>
                <div className={styles.label}>이미지 크기</div>
                <div className={styles.value}>{prop.width} X {prop.height}</div>
            </div>
            <div className={styles.card__infoBox__row}>
                <div className={styles.label}>업로드 날짜</div>
                <div className={styles.value}>{prop.created_at.split('T')[0]}</div>
            </div>
            <div className={styles.card__infoBox__row}>
                <div className={styles.label}>마지막 업데이트</div>
                <div className={styles.value}>{prop.updated_at.split('T')[0]}</div>
            </div>
            <div className={styles.card__infoBox__row}>
                <div className={styles.label}>다운로드 수</div>
                <div className={styles.value}>{prop.likes}</div>
            </div>
        </div>
    </div>
  )
}

export default Card